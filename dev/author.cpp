...
void __fastcall TF_Author::Timer1Timer(TObject *Sender)
{
    ...
    const float c_phase = 3.14;
    const float c_height = Height - c_rh*4 - 60;
    ...
    for( int i = 0; i < c_number; ++i ) {
        ...
        Canvas->Pixels
            ...
            [ 30 + i*c_height/c_number + c_rh * sin((time-1+i*1.5)/c_speed + dj) ]
            = clBlack;

        Canvas->Pixels
            [ Memo1->Width + 10 + c_r * ( 1 + cos( (time+i*1.5)/c_speed + dj) ) ]
            [ 30 + i*c_height/c_number  + c_rh * sin((time+i*1.5)/c_speed + dj) ]
            = clWhite;
        }
    }
